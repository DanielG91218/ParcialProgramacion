...
using DronParcial.Data;
using DronParcial.Domain;
using DronParcial.Services;
...
{
    Console.WriteLine("No existe la cadena ConnectionStrings:PostgreSql en appsettings.json.");
    return;
}
...
int size = ReadIntAtLeast("Ingrese la dimension del terreno N (>= 1): ", 1);
int startX = ReadCoordinate("Ingrese la coordenada inicial X / fila: ", size);
int startY = ReadCoordinate("Ingrese la coordenada inicial Y / columna: ", size);

var start = new Coordinate(startX, startY);
var solver = new DronePathSolver();
DroneRouteResult result = solver.Solve(size, start);
...
if (!result.Success)
{
    ...
    Console.WriteLine($"Desde ({start.X},{start.Y}) el dron alcanza {result.ReachableCount} parcela(s), pero no existe una ruta que las visite una sola vez.");
    return;
}

Console.WriteLine($"Recorrido exitoso: cubre {result.Route.Count} de {size * size} parcela(s).");
Console.WriteLine("Matriz del recorrido:");
PrintBoard(result.Board);

var repository = new DroneRepository(connectionString);

try
{
    int masterId = repository.SaveSuccessfulExecution(size, start, result.Route);
    Console.WriteLine();
    Console.WriteLine($"Simulacion guardada correctamente. ID generado: {masterId}");

    IReadOnlyList<MovementLogRow> lastRows = repository.GetLastFiveMovements(masterId);
    Console.WriteLine();
    Console.WriteLine("Ultimos 5 movimientos reconstruidos desde la base:");
    PrintReverseReport(lastRows);
}
catch (Exception ex)
{
    Console.WriteLine();
    Console.WriteLine("El recorrido se calculo, pero no se pudo persistir o consultar PostgreSQL.");
    Console.WriteLine("Revise que la base exista, que el script SQL se haya ejecutado y que appsettings.json tenga credenciales validas.");
    Console.WriteLine(ex.Message);
}
...
static int ReadIntAtLeast(string prompt, int minimum)
{
    while (true)
    {
        ...
        if (int.TryParse(value, out int number) && number >= minimum)
        {
            return number;
        }

        Console.WriteLine($"Debe ingresar un entero mayor o igual a {minimum}.");
    }
}

static int ReadCoordinate(string prompt, int size)
{
    while (true)
    {
        Console.Write(prompt);
        ...
        if (int.TryParse(value, out int coordinate) && coordinate >= 0 && coordinate < size)
        {
            return coordinate;
        }

        Console.WriteLine($"La coordenada debe estar en el rango [0, {size - 1}].");
    }
}
...
static void PrintBoard(int[,] board)
{
    ...
    int maxStep = FindMaxStep(board);
    int cellWidth = Math.Max(2, maxStep.ToString().Length + 1);
    ...
    while (x < size)
    {
        ...
        {
            int step = board[x, y];
            string text = step >= 0 ? step.ToString() : ".";
            Console.Write(text.PadLeft(cellWidth));
            y++;
        }
...
static int FindMaxStep(int[,] board)
{
    int max = 0;
    int x = 0;
...
static void PrintReverseReport(IReadOnlyList<MovementLogRow> rows)
{
    if (rows.Count == 0)
    {
    ...
    {
        MovementLogRow row = rows[index];
        Console.WriteLine($"BD id={row.Id} | paso guardado={row.SavedStep} | paso real={row.RealStep} | X={row.X} | Y={row.Y}");
        index++;
    }
}
